...
async function fetchEcosData(code: string, itemCode: string, period: string = 'D') {
  try {
    const now = new Date();
    // Use a wider range to ensure we get data even on weekends/holidays
    const today = now.toISOString().slice(0, 10).replace(/-/g, '');
    const ago = new Date(now.getTime() - 15 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10).replace(/-/g, '');
    
    const url = `https://ecos.bok.or.kr/api/StatisticSearch/${ECOS_API_KEY}/json/kr/1/10/${code}/${period}/${ago}/${today}/${itemCode}/`;
...
export async function getRealTimeFinanceData() {
  ...
  // Fetch all domestic data in parallel
  ...
  // US Market and others are still mock/static for now as we don't have free easy APIs for them in browser without CORS issues
  // But we return them so the structure remains same
  return {
    market: {
      usMarket: [
